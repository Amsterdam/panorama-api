...
class ManageMaterializedView(ManageView):
    def database_forwards(self, app_label, schema_editor, from_state, to_state):
        schema_editor.execute(
            "DROP MATERIALIZED VIEW IF EXISTS {}".format(self.view_name)
        )
        schema_editor.execute(
            "CREATE MATERIALIZED VIEW {} AS {}".format(self.view_name, self.sql)
        )
    ...
    def database_backwards(self, app_label, schema_editor, from_state, to_state):
        ...
        if previous:
            schema_editor.execute(
                "CREATE MATERIALIZED VIEW {} AS {}".format(self.view_name, previous)
            )
