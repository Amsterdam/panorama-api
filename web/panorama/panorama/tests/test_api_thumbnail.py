...
class ThumbnailApiTest(PanoramaApiTest):
    def test_get_thumbnail_returns_json(self):
        ...
        self.assertEqual(
            response.data["url"],
            "http://testserver/panorama/thumbnail/PANO_9_2017_CLOSE_BUT_NO_CIGAR/?heading=270",
        )
        self.assertEqual(response.data["heading"], 270)

    def test_get_thumbnail_returns_jpg(self):
        response = self.client.get(
            "/panorama/thumbnail/?lat=52.377956&lon=4.897070&radius=200",
            follow=False,
    ...
    def test_get_thumbnail_returns_json_with_qparams(self):
        response = self.client.get(
            "/panorama/thumbnail/?lat=52.377956&lon=4.897070&radius=200&width=600",
            follow=False,
            HTTP_ACCEPT="image/jpeg",
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn(
            "http://testserver/panorama/thumbnail/PANO_9_2017_CLOSE_BUT_NO_CIGAR/?",
            response["Location"],
        )
