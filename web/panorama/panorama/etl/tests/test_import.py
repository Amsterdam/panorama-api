...
def mock_get_csv_type(container, mission_path, csv_type):
    ...
    if csv_type == "panorama":
        files = glob.glob(
            f"/app/panoramas_test/{container}/{mission_path}panorama*.csv",
            recursive=True,
        )
    return [{"container": "1", "name": f} for f in files]
...
def mock_get_root_csvs(csv_type):
    ...
    return [
        {"container": "1", "name": f, "last_modified": last_modified_str} for f in files
    ]
...
@skipIf(
    not os.path.exists("/app/panoramas_test"),
...
class ImportPanoTest(TransactionTestCase):
    ...
    @mock.patch(
        mock_objectstore % "get_containerroot_csvs", side_effect=mock_get_root_csvs
    )
    @mock.patch(
        mock_objectstore % "get_panorama_store_object", side_effect=mock_get_csv
    )
    @mock.patch(
        "panorama.etl.data_to_model.panorama_image_file_exists",
        side_effect=mock_file_exists,
    )
    @mock.patch(
        "panorama.etl.data_to_model.panorama_blurred_file_exists",
        side_effect=lambda a, b, c: blurred_file_exists,
    )
    @mock.patch(
        "panorama.etl.data_to_model.panorama_rendered_file_exists",
        side_effect=lambda a, b, c: rendered_file_exists,
    )
    def test_import(self, *args):
        import_mission_metadata()
        for (container, mission_path) in [
        ...
        self.assertEqual(
            Mission.objects.filter(mission_year="2016", mission_type="woz").count(), 2
        )
        self.assertEqual(Mission.objects.filter(mission_year="2017").count(), 1)
        self.assertEqual(
            Mission.objects.filter(mission_year="2017", mission_type="bi").count(), 0
        )
        ...
        self.assertAlmostEqual(
            panos[0]._geolocation_2d_rd,
            panos[0]._geolocation_2d.transform(28992, clone=True),
        )
        ...
        self.assertEqual(
            Panoramas.objects.filter(pano_id="TMX7315120208-000032_pano_0000_007572")[
                0
            ].surface_type,
            "L",
        )
        self.assertEqual(
            Panoramas.objects.filter(pano_id="TMX7315120208-000033_pano_0000_006658")[
                0
            ].surface_type,
            "W",
        )
    ...
    def test_panoramarow_sets_status(self, *args):
        ...
        row_to_be_rendered = {
            "panorama_file_name": "row_to_be_rendered",
            **basic_row_fields,
        }
        actual = process_panorama_row(
            row_to_be_rendered.keys(),
