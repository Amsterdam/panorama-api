...
def process_csv(csv_file, generate_model):
    ...
    rows = csv.reader(
        csv_file_iterator, delimiter="\t", quotechar=None, quoting=csv.QUOTE_NONE
    )
...
def import_mission_metadata():
    ...
    for csv_file in objectstore.get_containerroot_csvs("missiegegevens"):
        last_modified_dt = datetime.strptime(
            csv_file["last_modified"][:19], "%Y-%m-%dT%H:%M:%S"
        )
        log.info(
            f"READING {csv_file['container']} {csv_file['name']} ({last_modified_dt})"
        )
        Mission.objects.bulk_create(
            process_csv(csv_file, process_mission_row), batch_size=BATCH_SIZE
        )
...
def rebuild_mission(container, mission_path):
    ...
    for csv_file in panorama_csvs:
        ...
        Panoramas.objects.bulk_create(
            process_csv(csv_file, process_panorama_row), batch_size=BATCH_SIZE
        )
