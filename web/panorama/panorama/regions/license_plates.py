...
def parse(results, x, y, zoom, shear_angle, widen):
    ...
    for result in results:
        ...
        if "III" not in result["plate"]:
            parsed.append(
                derive(
                    result["coordinates"], x, y, zoom, shear_angle, widen, detected_by
                )
            )
    return parsed
    ...
    def get_licenseplate_regions(self):
        ...
        for x in range(0, Img.PANORAMA_WIDTH, SAMPLE_DISTANCE_H):
            for idx, y in enumerate(
                range(JUST_BELOW_HORIZON, PLATES_NEAR_BY, SAMPLE_DISTANCE_V)
            ):
                ...
                results = alpr.recognize_array(Img.image2byte_array(zoomed_snippet))[
                    "results"
                ]
                ...
                for angle in ANGLE_RANGE:
                    for direction in [1, -1]:
                        widen, size, affine_matrix = calculate_shear_data(
                            angle * direction
                        )
                        sheared = snippet.transform(
                            size, AFFINE, affine_matrix, BICUBIC
                        )
                        resized = resize_sheared(sheared, size, widen, zoom)
                        results = alpr.recognize_array(Img.image2byte_array(resized))[
                            "results"
                        ]
                        licenseplate_regions.extend(
                            parse(results, x, y, zoom, angle * direction, widen)
                        )
