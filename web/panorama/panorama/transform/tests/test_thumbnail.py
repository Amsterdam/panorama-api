...
from panorama.transform import thumbnail
...
class TestThumb(TestCase):
    def test_source_selection(self):
        ...
        self.assertEqual(
            thumbnail.choose_source_image_and_width(fov, sample_width), ("full", 8000)
        )
        self.assertEqual(
            thumbnail.choose_source_image_and_width(fov, sample_width / 2),
            ("medium", 4000),
        )
        self.assertEqual(
            thumbnail.choose_source_image_and_width(fov, sample_width / 4),
            ("small", 2000),
        )
    ...
    def test_calculate_crop(self):
        ...
        self.assertEqual(
            actual_crop[0], int(1000 + heading * pixels_per_degree - 1000 * 80 / 360)
        )
        self.assertEqual(actual_crop[1], 351)
        self.assertAlmostEqual(
            actual_crop[2], int(actual_crop[0] + 2000 * 80 / 360), delta=1
        )
        ...
        self.assertAlmostEqual(
            actual_crop[2] - actual_crop[0], 4000 * 80 / 360, delta=1
        )
        self.assertAlmostEqual(
            (actual_crop[2] - actual_crop[0]) / (actual_crop[3] - actual_crop[1]),
            aspect,
            delta=0.01,
        )
        ...
        self.assertAlmostEqual(
            actual_crop[2] - actual_crop[0], 4000 * fov / 360, delta=1
        )
        ...
        self.assertAlmostEqual(
            actual_crop[1], int(1000 - (1 - horizon) * height), delta=1
        )
