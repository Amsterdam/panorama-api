...
def calculate_crop(
    source_width, target_fov, target_aspect, target_heading, target_horizon
):
    """
...
class Thumbnail(object):
    ...
    def get_image(
        ...
        """
        source_image, source_width = choose_source_image_and_width(
            target_fov, target_width
        )
        ...
        images = {
            "full": self.panorama.equirectangular_full,
        ...
        if right > source_width:
            ...
            crop_wrapped = (
                0 if left <= source_width else left - source_width,
                top,
                right - source_width,
                bottom,
            )
            ...
            thumbnail.paste(
                wrap_img,
                (width - (crop_wrapped[2] - crop_wrapped[0]), 0, width, height),
            )

        # resize thumbnail to requested width
        return thumbnail.resize(
            (target_width, int(target_width / target_aspect)), BICUBIC
        )
