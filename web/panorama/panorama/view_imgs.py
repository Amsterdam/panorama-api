...
class ThumbnailViewSet(PanoramaViewSet):
    ...
    def list(self, request, **kwargs):
        ...
        image_redirect = (
            "image/" in request.accepted_renderer.media_type
        ...
        if image_redirect:
            ...
            resp = ThumbnailSerializer(
                {"url": url, "heading": heading, "pano_id": pano_id},
    ...
    def _get_thumb_url(self, pano_id, heading, request):
        ...
        parameters.update(
            {k: v for k, v in request.query_params.items() if k not in _strip_params}
        )
