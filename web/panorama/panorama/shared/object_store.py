...
class ObjectStore:
    ...
    def __init__(self):
        ...
        self.datapunt_conn = client.Connection(
        ...
        self.panorama_conn = client.Connection(
            authurl=settings.AUTHURL,
    ...
    def get_panorama_store_object(self, object_meta_data):
        try:
            return self.panorama_conn.get_object(
                object_meta_data["container"], object_meta_data["name"]
            )[1]
        except ClientException as exc:
            log.error(exc)
            log.error(
                "User: %s (%s)",
                settings.PANORAMA_OBJECTSTORE_USER,
                settings.PANORAMA_TENANT_NAME,
            )
            raise

    def get_panorama_store_objects(self, container, path):
        return self._get_full_container_list(
            self.panorama_conn, container, [], prefix=path
        )

    def get_datapunt_store_objects(self, path):
        return self._get_full_container_list(
            self.datapunt_conn, settings.DATAPUNT_CONTAINER, [], prefix=path
        )
    ...
    def _get_full_container_list(self, conn, container, seed, **kwargs):
        ...
        return (
            seed
            if len(page) < self.RESP_LIMIT
            else self._get_full_container_list(conn, container, seed, **kwargs)
        )
            ...
            for month in self.get_subdirs(container, ""):
                for day in self.get_subdirs(container, month):
                    for trajectory in self.get_subdirs(container, day):
                        csvs.extend(
                            self.get_csv_type(container, trajectory, csv_identifier)
                        )
        return csvs

    def get_subdirs(self, container, path):
        objects_from_store = self._get_full_container_list(
            self.panorama_conn, container, [], delimiter="/", prefix=path
        )
        return [
            store_object["subdir"]
            for store_object in objects_from_store
    ...
    def get_csv_type(self, container, path, csv_identifier):
        csvs = self._get_full_container_list(
            self.panorama_conn,
            container,
            [],
            delimiter="/",
    ...
    def put_into_panorama_store(
        self, container, object_name, object_content, content_type
    ):
        self.panorama_conn.put_object(
